...
class NNmodel(object):
    def __init__(
        ...
        self.epoch = 80 if 'epoch' not in parameters.keys() else parameters['epoch']
    ...
    def res_net_block(self, inputs):
        for layer in range(0, self.layer):
            x = tf.keras.layers.Dense(self.node, activation=self.activation,
                   kernel_regularizer=tf.keras.regularizers.l2(self.l2))(inputs)
            x = tf.keras.layers.BatchNormalization()(x)
            x = tf.keras.layers.Dropout(self.dropout)(x)
        x = tf.keras.layers.Dense(self.nr_trainvars,
                     kernel_regularizer=tf.keras.regularizers.l2(self.l2))(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Dropout(self.dropout)(x)
    ...
    def makeResNet(self, inputs):
        res_inputs = inputs
        for reslayer in (0, self.ResNetLayer):
...
class LBNmodel(NNmodel):
    ...
    def fit_model(self):
        history = self.model.fit(
            ...
            sample_weight=self.train_data[self.weight].values,
            validation_data=(
                self.val_data[self.trainvars].values,
                self.val_target,
                self.val_data[self.weight].values
            ),
            callbacks=[self.reduce_lr, self.early_stopping]
    ...
    def create_model(self):
        ...
        lbn_features = lbn_layer(dlt.get_low_level(self.train_data, self.particles[self.channel]))
